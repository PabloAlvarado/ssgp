...
def ss_gp(gpu, inst):
    ...
    """Source separation using gaussian processes"""
    # initialize settings
    path = '/import/c4dm-04/alvarado/datasets/ss_amt/'  # data location
    sess = gpitch.init_settings(gpu)  # select gpu

    # load training data
    xtrain, ytrain, fs, tfn = srcs.load_traindata(path=path + 'training_data/', inst=inst)  # tfn = train file names
    nsrc = len(ytrain)  # number of sources

    # get kernel features

    # load test data

    # split data inot windows
    ...
    for i in range(nw):
        pass
        # reset model if is not the first time
        # optimize
        # predict
        # save partial prediction

    # joint all partial results
    ...
    for i in range(nsrc):
        ...
        # plt.plot(other[i][2], other[i][3], 'xC0')
        # plt.plot(freqf[i], varf[i], 'vC1')
        # plt.legend(["data spetral density ", "selected features"])
